"""
Genereer een fictief selectiebestand voor Psychologie (demo Radboud).

Structuur lijkt op de nieuwste Psychologie-selectiedata: schoolcijfers met
drie kernvakken, een combinatiecijfer en een matchingsvragenlijst. De
...
200 kandidaten, waarvan 70 ingeschreven. Niet herleidbaar naar echte data.

Draai:
    uv run python scripts/eenmalig/maak_fictief_demo_radboud.py
"""
...
# Drie kernvakken met cijfers en puntenscore, net als de Psychologie-data
kernvakken = {
    "Wiskunde": {"afk": "WI", "gem": 6.4, "std": 1.0},
    "Engels": {"afk": "EN", "gem": 7.0, "std": 0.85},
    "Biologie": {"afk": "BIO", "gem": 6.6, "std": 0.95},
}
...
matching_score = RNG.choice([1, 2, 3], size=N, p=[0.18, 0.42, 0.40]).astype(float)
...
xlsx_path = OUT_DIR / "selectiedata_demo_radboud_2026.xlsx"
...
# -- Configuratiebestand ------------------------------------------------------
config_path = OUT_DIR / "config_demo_radboud_2026.xlsx"
make_config(
    str(config_path),
    [
...
# -- Kopieer naar demo --------------------------------------------------------
demo_subdir = Path("data/demo/demo_radboud_2026")
